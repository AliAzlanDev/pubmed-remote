...
const IS_REMOTE = process.env.MCP_TRANSPORT === "sse" || process.argv.includes("--remote");
  ...
  try {
    ...
    switch (name) {
      ...
      case "find_similar_articles": {
        const { pmid, maxResults = 10 } = args as { pmid: string; maxResults?: number };
        ...
        articles.forEach((article, index) => {
          ...
          if (article.score) {
            responseText += `Similarity Score: ${article.score}\n`;
          }
      ...
      case "batch_process": {
        let { pmids, operations, maxConcurrency = 3 } = args as {
          pmids: string[] | string;
          operations: string[];
  ...
  try {
    if (IS_REMOTE) {
      ...
      app.listen(PORT, HOST, () => {
        log("info", `PubMed MCP Server v1.0.2 is running on http://${HOST}:${PORT}`);
